...
error InvalidWalkingSpeedDataLength();
error InvalidHiitDataLength();
...
contract ChallengeBaseStep is IERC721Receiver {
    ...
    */
    ...
    // Gas DoS guards for daily-result and settlement array inputs.
    ...
    function sendDailyResult(
        ...
        _executeDailyResult(_day, _stepIndex, _data, _signature, _listGachaAddress, _listNFTAddress, _listIndexNFT, _listSenderAddress, _statusTypeNft, _timeRange, _intervals, _totalSeconds, _minutesAtTargetSpeed, _metsWalkingSpeed);
    }
    ...
    /**
     * @notice Meta-tx: challenger UỶ QUYỀN settlement bằng CHỮ KÝ (raw key KHÔNG rời client), relayer submit.
     *         ⚠️ FUND-AUTHORIZATION — BẮT BUỘC security audit trước khi deploy.
     */
    function sendDailyResultViaRelayer(
        ...
        bytes32 payload = keccak256(
            abi.encode(address(this), block.chainid, _nonce, _deadline, _day, _stepIndex, _data, _timeRange, _intervals, _totalSeconds, _minutesAtTargetSpeed, _metsWalkingSpeed, assetHash)
        );
    ...
    // ecrecover + chống malleability (EIP-2). ⚠️ audit kỹ.
    ...
    function _executeDailyResult(
        ...
        if (isHiitEnabled) {
            if (!(_intervals.length == dayLength && _totalSeconds.length == dayLength)) revert InvalidHiitDataLength();
        }
     ...
     * deposited, or already returned) is skipped rather than reverting the whole batch, so one
     * bad/stale entry can't block the rest of a legitimate settlement.
     * @param _listNFTAddress An array of NFT contract addresses.
     * @param _listIndexNFT An array of arrays containing indices of NFTs to transfer.
        ...
        for (uint256 i = 0; i < _listNFTAddress.length; i++) {
            if (_statusTypeNft[i]) {
                for (uint256 j = 0; j < _listIndexNFT[i].length; j++) {
                    ...
                    // No safeTransferFrom deposit record exists -- e.g. deposited via plain
                    // transferFrom, which never triggers onERC721Received. Fall back to the
                    // challenge's own AUTHORITATIVE `challenger` address (never a caller-supplied
                    // one) if this contract still actually holds the token; never to an arbitrary
                    // address chosen by whoever calls this function.
                ...
                for (uint256 j = 0; j < lengthListIndexNFT; j++) {
                    ...
                    if (depositors.length > 0) {
                        ...
                        // No onERC1155Received record exists for this tokenId. Fall back to the
                        // challenge's own AUTHORITATIVE `challenger` address for whatever balance
                        // this contract actually holds -- never a caller-supplied address/amount.
