...
contract ChallengeGCMAndSpeed is IERC721Receiver {
    ...
    */
    ...
    function sendDailyResult(
        ...
        _executeDailyResult(_day, _stepIndex, _data, _signature, _listGachaAddress, _listNFTAddress, _listIndexNFT, _listSenderAddress, _statusTypeNft, _timeRange, _minutesAtTargetSpeed, _metsWalkingSpeed, _glucoseLevels);
    }
    ...
    function sendDailyResultViaRelayer(
        ...
        bytes32 payload = keccak256(
            abi.encode(address(this), block.chainid, _nonce, _deadline, _day, _stepIndex, _data, _timeRange, _minutesAtTargetSpeed, _metsWalkingSpeed, _glucoseLevels, assetHash)
        );
    ...
    function _executeDailyResult(
        ...
        // _glucoseLevels into the signed payload -- previously accepted unsigned after the
        // common signature check, letting a challenger/relayer alter achievement evidence
        // post-signing without invalidating the signature.
        IExerciseSupplementNFT(erc721Address[0]).checkValidSignature(
            _day,
            _stepIndex,
            _data,
            keccak256(abi.encode(_minutesAtTargetSpeed, _metsWalkingSpeed, _glucoseLevels)),
            _signature
        );
