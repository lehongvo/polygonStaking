...
// Verbatim copy of contracts/PolygonDeFiAggregator.sol as it existed at commit 342f0b1^ (the
// last commit before the CHALLENGE-2697 governed fee-recipient change), with only the outer
// contract name changed so it can compile alongside the real contract. This mirrors the storage
// layout ACTUALLY live on Polygon mainnet today (see deployInfo/polygon-defi-upgrade.json --
// newImplementation 0x17aC91ce... was deployed 2025-10-23, before this file's variable was
// introduced). Used only by test/security/storage-layout-upgrade.test.ts via
// upgrades.validateUpgrade() to prove the current contracts/PolygonDeFiAggregator.sol is
// upgrade-safe from this exact on-chain baseline. Never deploy this contract for any other
// purpose.
 ...
 */
contract OldDefiV1ForLayoutTest is
    Initializable,
    OwnableUpgradeable,
