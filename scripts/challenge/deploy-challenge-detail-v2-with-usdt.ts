...
async function main() {
  console.log('🚀 CHALLENGE DETAIL V2 DEPLOYMENT (USDT)');
  console.log('==========================================');
  ...
  try {
    const envConfigRaw = process.env.CONFIG_DEPLOY_CHALLENGE_USDT;
  ...
  if (config.createByToken !== hre.ethers.ZeroAddress) {
    ...
    if (contractUsdtBalance >= config.totalAmount) {
      ...
      console.log('⚠️ USDT staking may have failed');
    }
  }
  ...
  if (config.createByToken !== hre.ethers.ZeroAddress) {
    ...
    if (contractUsdtBalance >= config.totalAmount) {
      ...
      console.log('⚠️ USDT transfer may have failed');
    }
  }
  ...
  const outputPath = path.join(
    process.cwd(),
    `deployInfo/challenge-detail-v2-usdt-${network.name}.json`
  );
  ...
  console.log(`Contract: ChallengeDetailV2 (USDT)`);
