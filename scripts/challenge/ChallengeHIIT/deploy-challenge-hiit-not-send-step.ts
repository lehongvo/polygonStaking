...
/**
 * Deploy Challenge HIIT without sending step (deploy + verify + grant role only).
 * - Default: deploy → verify → grant role → save deployInfo. No send step.
 ...
 *   npx hardhat run scripts/challenge/ChallengeHIIT/deploy-challenge-hiit-not-send-step.ts --network polygon
 *
 * Example (grant role only):
 *   NOT_DEPLOY=true npx hardhat run scripts/challenge/ChallengeHIIT/deploy-challenge-hiit-not-send-step.ts --network polygon
 */
...
async function main() {
  ...
  console.log('🚀 CHALLENGE HIIT DEPLOYMENT — NO SEND STEP');
  console.log('===========================================');
