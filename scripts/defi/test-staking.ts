...
async function main() {
  ...
  const polTokenAddress =
    deploymentInfo.tokens?.POL || process.env.POL_TOKEN_ADDRESS;
  if (!polTokenAddress) {
  ...
  if (polBalance < stakeAmount) {
    console.error(
      `❌ Insufficient POL balance! Need 0.1 POL, have ${hre.ethers.formatEther(polBalance)} POL`
    );
    process.exit(1);
  }
  ...
  try {
    ...
    const approveTx = await polToken.approve(
      deploymentInfo.contractAddress,
      stakeAmount
    );
    ...
    if (stakeId) {
      ...
      console.log(
        `POL Balance Before Withdraw: ${hre.ethers.formatEther(balanceBefore)} POL`
      );
      ...
      console.log(
        `POL Balance After Withdraw: ${hre.ethers.formatEther(balanceAfter)} POL`
      );
    ...
    if (deploymentInfo.explorer) {
      ...
      if (stakeId) {
        console.log(
          `- Withdraw: ${deploymentInfo.explorer}/tx/${withdrawTx.hash}`
        );
      }
    }
  } catch (error: any) {
    ...
    if (error.message.includes('Time lock not expired')) {
      console.log(
        '💡 Time-lock has not expired yet. Wait longer before withdrawing.'
      );
    } else if (error.message.includes('insufficient allowance')) {
      console.log(
        '💡 Token approval failed. Check token balance and try again.'
      );
    } else if (error.message.includes('Stake not found')) {
      console.log(
        '💡 Stake ID not found. Check if stake was created successfully.'
      );
    }
