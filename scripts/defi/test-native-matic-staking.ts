...
async function main() {
  ...
  const deploymentData = JSON.parse(
    fs.readFileSync('./deployInfo/polygon-defi-deployment.json', 'utf8')
  );
  ...
  console.log(
    "   contract.stake(WMATIC_ADDRESS, 0, 'aave_lending', { value: ethers.parseEther('1.0') })"
  );
  console.log('');

  console.log('💡 How Native MATIC Staking Works:');
  console.log(
    '   1. User calls stake() with WMATIC address + sends native MATIC'
  );
  ...
  if (balance > testAmount * 2n) {
    ...
    try {
      ...
      console.log(
        `📊 Position Before: ${ethers.formatEther(positionBefore.balance)} WMATIC`
      );

      // Execute native MATIC staking
      console.log(
        `💸 Staking ${ethers.formatEther(testAmount)} native MATIC...`
      );
      ...
      console.log(
        `📊 Position After: ${ethers.formatEther(positionAfter.balance)} WMATIC`
      );
      ...
      console.log(
        '💡 This might be expected if Aave protocol is not set up yet'
      );
    }
  } else {
