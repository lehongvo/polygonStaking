...
async function main() {
  ...
  if (balance < ethers.parseEther('0.01')) {
    console.log(
      '❌ Insufficient balance for deployment. Need at least 0.01 ETH/MATIC'
    );
    process.exit(1);
  }
  ...
  // Verify protocols
  const [protocolNames, apys, activeStatus] =
    await aggregator.getAllProtocols();
  console.log(`📋 Protocols: ${protocolNames.join(', ')}`);

  // Verify tokens
  const [tokenAddresses, symbols, decimals, tokenActiveStatus] =
    await aggregator.getAllSupportedTokens();
  ...
  try {
    ...
    console.log(
      `✅ Test stake created! Gas used: ${stakeReceipt.gasUsed.toString()}`
    );

    // Get user stakes
    const userStakes = await aggregator.getUserTimeLockedStakes(
      deployer.address
    );
    ...
    if (userStakes.length > 0) {
      ...
      console.log(
        `📋 Latest stake: ${ethers.formatEther(latestStake.amount)} MATIC for ${latestStake.protocol}`
      );
      ...
      console.log(
        `✅ Withdrawal successful! Gas used: ${withdrawReceipt.gasUsed.toString()}`
      );
    }
  } catch (error: any) {
