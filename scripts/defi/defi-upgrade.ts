...
async function main() {
  ...
  if (balance < ethers.parseEther("0.01")) {
    console.log("❌ Insufficient balance for upgrade. Need at least 0.01 ETH/MATIC");
    process.exit(1);
  }
  
  // Load existing deployment info
  const deploymentFile = network.name === "polygon" ? "polygon-defi-aggregator.json" : "hardhat-defi-aggregator.json";
  const deploymentPath = path.join(__dirname, "../../deployInfo", deploymentFile);
  ...
  const proxyContract = await ethers.getContractAt("PolygonDeFiAggregator", proxyAddress);
  ...
  if (network.name === "polygon") {
    try {
      ...
      verificationNote = "New implementation verified successfully on PolygonScan";
  ...
  // Save upgrade info
  const upgradeFileName = network.name === "polygon" ? "polygon-defi-upgrade.json" : "hardhat-defi-upgrade.json";
