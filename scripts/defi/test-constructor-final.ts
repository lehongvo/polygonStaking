...
async function main() {
  ...
  const ConstructorStakeExample = await ethers.getContractFactory(
    'ConstructorStakeExample'
  );
  ...
  console.log(
    '📋 Staked Amount:',
    formatEther(await constructorStake.stakedAmount()),
    'MATIC'
  );
  ...
  try {
    ...
    if (stakes.length > 0) {
      ...
      console.log(
        '  - End Time:',
        new Date(Number(stake.endTime) * 1000).toISOString()
      );
  ...
  try {
    ...
    console.log(
      'Amount withdrawn:',
      formatEther(balanceAfter - balanceBefore),
      'MATIC'
    );
  } catch (error) {
  ...
  console.log(
    ...
    formatEther(finalContractBalance),
    'MATIC'
  );
