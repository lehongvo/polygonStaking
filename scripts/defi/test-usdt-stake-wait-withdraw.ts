...
async function main() {
  ...
  const deploymentInfo = JSON.parse(
    fs.readFileSync(`${process.cwd()}/deployInfo/polygon-defi-aggregator.json`, "utf8")
  );
  const contractAddress: string = deploymentInfo.contractAddress;
  console.log("📋 Aggregator:", contractAddress);

  const aggregator = await ethers.getContractAt("PolygonDeFiAggregator", contractAddress);
  ...
  const stakeGas = await aggregator.createTimeLockedStake.estimateGas(USDT, amount, "aave_lending", lock);
  console.log("⛽ Estimated gas (stake):", stakeGas.toString());
  const tx = await aggregator.createTimeLockedStake(USDT, amount, "aave_lending", lock, {
  ...
  console.log("⏳ Waiting ~60 seconds before withdrawing (contract doesn't enforce maturity on withdraw)...");
  await new Promise((r) => setTimeout(r, 60_000));

  console.log("\n💰 Withdrawing...");
  try {
    // Estimate gas for withdrawal
    const withdrawGas = await aggregator.withdrawTimeLockedStake.estimateGas(stakeId, deployer.address);
    console.log("⛽ Estimated gas (withdraw):", withdrawGas.toString());
    const wtx = await aggregator.withdrawTimeLockedStake(stakeId, deployer.address, {
    ...
    for (const log of wrc.logs) {
      try {
        ...
        if (parsed?.name === "TimeLockedStakeWithdrawn") {
          console.log("📊 amountWithdrawn:", ethers.formatUnits(parsed.args.amountWithdrawn, 6), "USDT");
          console.log("📈 rewards:", ethers.formatUnits(parsed.args.rewards, 6), "USDT");
          console.log("🏷️ systemFee:", ethers.formatUnits(parsed.args.systemFee, 6), "USDT");
        }
      } catch {}
    }
  } catch (e: any) {
