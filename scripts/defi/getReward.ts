...
async function main() {
  ...
  const user = ethers.isAddress(addr) ? addr : "0x980B8Cd287b01deAeAD8576846eEb6de7Bcc2A50";

  const provider = ethers.provider;
  const controller = new ethers.Contract(AAVE_REWARDS_CONTROLLER, ABI, provider);
  ...
  try {
    ...
    console.log("getRewardsByAsset not available on this controller (likely v2)");
  }
  ...
  if (unclaimed !== null) {
    console.log(`Unclaimed rewards: ${ethers.formatEther(unclaimed)} (WMATIC program on Polygon)`);
  } else {
    console.log(`Unable to query rewards with available ABIs (controller address/ABI mismatch).`);
  }
  ...
  try {
    ...
    const interestWei = currentValueWei > principalWei ? (currentValueWei - principalWei) : 0n;

    console.log(`aToken scaledBalance: ${scaled.toString()}`);
    console.log(`Current underlying value (from aToken.balanceOf): ${ethers.formatEther(currentValueWei)} WMATIC`);
    if (principalWei > 0n) {
      console.log(`Interest (vs principal ${principalEnv}): ${ethers.formatEther(interestWei)} WMATIC`);
    }
  } catch (e) {
