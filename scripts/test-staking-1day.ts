...
async function main() {
  ...
  try {
    ...
    if (stakeId) {
      ...
      console.log(`1. Wait until: ${new Date(endTime * 1000).toLocaleString()}`);
      console.log(`2. Call withdrawTimeLockedStake(${stakeId}) on the contract`);
      console.log(`3. Or use: npx hardhat run scripts/withdraw-stake.ts --network polygon`);
