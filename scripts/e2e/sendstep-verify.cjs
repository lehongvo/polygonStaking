...
async function main() {
  ...
  assertEq(
    s1.currentStatus,
    1n,
    '[F-A7-equiv] currentStatus = 1 (1/2 days passed)'
  );
  assertEq(
    s1.isFinished,
    false,
    "isFinished still false (CEI doesn't fire prematurely)"
  );
  ...
  console.log(
    '  history.dates:',
    history[0].map(x => x.toString())
  );
  console.log(
    '  history.steps:',
    history[1].map(x => x.toString())
  );
  assertEq(
    history[0].length,
    1,
    '[N2 reset OK] historyDate length = 1 after 1 call'
  );
  assertEq(
    history[1].length,
    1,
    '[N2 reset OK] historyData length = 1 after 1 call'
  );
  ...
  const data2 = [
    0,
    (await hre.ethers.provider.getBlock('latest')).timestamp + 100,
  ];
  ...
  assertEq(
    s2.isFinished,
    true,
    '[F-A5 CEI] isFinished = true after success path'
  );
  assertEq(
    s2.isSuccess,
    true,
    '[F-A5 CEI] isSuccess = true after success path'
  );
  ...
  console.log(
    '  recv1 (success-side) gained:',
    hre.ethers.formatEther(recv1Got),
    'ETH'
  );
  console.log(
    '  feeAddr gained:              ',
    hre.ethers.formatEther(feeGot),
    'ETH'
  );
  assertTrue(
    recv1Got > 0n,
    '[F-A7] recv1 (success-side, percent=50) received payout'
  );
  ...
  console.log(
    '  recv2 (fail-side) total balance:',
    hre.ethers.formatEther(balRecv2),
    'ETH'
  );
  ...
  try {
    ...
    if (e.message.includes('Challenge was finished')) {
      ...
      console.log(
        green('  ✓'),
        '[F-A4 + available modifier] revert with "Challenge was finished"'
      );
    } else {
      failChecks++;
      totalChecks++;
  ...
  const balUserBefore = await hre.ethers.provider.getBalance(
    challenger.address
  );
  ...
  console.log(
    '  Net user loss (excl gas):',
    hre.ethers.formatEther(netLoss - gasCost),
    'ETH'
  );
  assertTrue(
  ...
  console.log(
    `SUMMARY: ${passChecks}/${totalChecks} checks passed (${failChecks} failed)`
  );
  console.log(
    '================================================================'
  );
