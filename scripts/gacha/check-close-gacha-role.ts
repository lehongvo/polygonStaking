...
const CLOSE_GACHA_ROLE = ethers.id('CLOSE_GACHA_ROLE');
...
async function main() {
  console.log('🔐 GACHA CLOSE_GACHA_ROLE CHECK');
  console.log('===============================');
  ...
  console.log(`🔑 Role:    CLOSE_GACHA_ROLE = ${CLOSE_GACHA_ROLE}`);
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    try {
      const hasRole: boolean = await c.hasRole(CLOSE_GACHA_ROLE, target);
      const icon = hasRole ? '✅' : '🔒';
      console.log(
  ...
  if (missing > 0) {
    ...
    console.log(
      '\n   → withdrawBalances() will REVERT on these proxies for target wallet.'
    );
    console.log(
      '   → To grant, call grantRole(CLOSE_GACHA_ROLE, ' +
        target +
  ...
  console.log(
    '\n🎉 ALL PROXIES HAVE CLOSE_GACHA_ROLE — safe to call withdrawBalances.'
  );
}
