...
async function getExerciseSupplementImplementations(): Promise<void> {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    const tag = `[${i + 1}/${entries.length}] ${entry.Name} (${entry.ExerciseSupplementNFTProxyAddress})`;
    ...
    try {
      ...
      const impl = await readImplementationAddress(
...
if (require.main === module) {
  getExerciseSupplementImplementations().catch(err => {
    console.error('Fatal:', err);
    process.exit(1);
  });
}

export { getExerciseSupplementImplementations };
