...
const NEW_IMPL_ADDRESS = '0x86260443f2b8b2ea7637B67f3841E45B2A1e4c10';
...
async function main() {
  console.log('🚀 EXERCISE SUPPLEMENT NFT — PROXY UPGRADE');
  console.log('==========================================');
  ...
  const implInfoPath = path.join(
    process.cwd(),
    'deployInfo',
    'exercise-supplement-impl-polygon.json'
  );
  let newImplDeployAt = '';
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    try {
      currentImpl = await readImplFromProxy(
    ...
    if (currentImpl.toLowerCase() === NEW_IMPL_ADDRESS.toLowerCase()) {
      ...
      console.log(`⏭️  ${tag} — already at new impl, skip`);
      continue;
    }
  ...
  console.log(`Skip:     ${skipped} (already at new impl)`);
  ...
  const sampleContract = new ethers.Contract(
    ready[0].entry.ExerciseSupplementNFTProxyAddress,
    UPGRADE_ABI,
    deployer
  );
  ...
  if (balance < estCostTotal) {
    console.error(
      `❌ Balance ${ethers.formatEther(balance)} < estimated total ${ethers.formatEther(estCostTotal)}`
    );
    process.exit(1);
  }
  ...
  for (const state of ready) {
    ...
    console.log(
      `\n🔄 ${tag} (${state.entry.ExerciseSupplementNFTProxyAddress})`
    );

    const proxyContract = new ethers.Contract(
      state.entry.ExerciseSupplementNFTProxyAddress,
      UPGRADE_ABI,
      deployer
    );
    ...
    try {
      ...
      const newImpl = await readImplFromProxy(
        state.entry.ExerciseSupplementNFTProxyAddress
      );
      ...
      console.log(`   ✅ EIP-1967 slot now points to new impl`);

      // Post-upgrade safety check: securityAddress must be set
      let secAddr = '';
      try {
        secAddr = await readSecurityAddress(
          state.entry.ExerciseSupplementNFTProxyAddress
        );
        if (secAddr === ethers.ZeroAddress) {
          console.warn(
            `   🚨 CRITICAL: securityAddress is ZERO. Signature-based mints will revert with 'SECURITY ADDR NOT SET'.`
          );
          console.warn(
            `   🚨 Run IMMEDIATELY: proxy.updateSecurityAddress(<your signer>)`
          );
        } else {
          console.log(`   ✅ securityAddress: ${secAddr}`);
        }
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        console.warn(
          `   ⚠️  Could not read securityAddress (continuing): ${msg}`
        );
      }
      ...
      results.push({
        ...
        proxy: state.entry.ExerciseSupplementNFTProxyAddress,
        status: 'success',
        tx: tx.hash,
        gas: gasUsed,
  ...
  const reportPath = path.join(
    process.cwd(),
    'deployInfo',
    `exercise-supplement-upgrade-polygon-${Date.now()}.json`
  );
  const report = {
  ...
  console.log(`⏭️  Skipped:     ${skipped} (already at new impl)`);
  console.log(`🔒 No role:     ${noRole}`);
  console.log(`❌ Invalid:     ${invalid}`);
  ...
  if (success > 0) {
    console.log('\n🎯 CRITICAL FOLLOW-UPS (manual)');
    console.log('================================');
    console.log(
      '1. 🔴 If securityAddress was ZERO above, IMMEDIATELY call:'
    );
    console.log(
      '     proxy.updateSecurityAddress(<your backend signer address>)'
    );
    console.log(
      '   Otherwise all signature-based mints will revert with "SECURITY ADDR NOT SET".'
    );
    console.log(
      '2. 🟡 Ensure BACKEND wallet has UPDATER_ACTIVITIES_ROLE (needed for batchGrantRole).'
    );
    console.log(
      '3. 🟢 Optional: re-run scripts/exerciseSupplementNFT/get-implementation.ts'
    );
    console.log(
      '     to refresh ImplementAddress field with the now-current impl.'
    );
  }
