...
// _intervals, _totalSeconds, _minutesAtTargetSpeed, _metsWalkingSpeed.
async function buildSig(signer: any, addr: string, chainId: bigint, nonce: number, deadline: number,
  ...
  const payload = hre.ethers.keccak256(
    coder.encode(
      ['address', 'uint256', 'uint256', 'uint256', 'uint256[]', 'uint256[]', 'uint64[2]', 'uint64[2]', 'uint256[]', 'uint256[]', 'uint256[]', 'uint256[]'],
      [addr, chainId, nonce, deadline, day, stepIndex, data, timeRange, intervals, totalSeconds, minutesAtTargetSpeed, metsWalkingSpeed]
    )
  );
