...
import { MockAavePool, MockERC20, PolygonDeFiAggregatorSecure } from "../typechain-types";
...
describe("PolygonDeFiAggregatorSecure - Comprehensive Security Tests", function () {
    ...
    async function deployFixture() {
        ...
        const mockToken = await MockERC20Factory.deploy("Test Token", "TEST", INITIAL_SUPPLY);
        ...
        const ContractFactory = await ethers.getContractFactory("PolygonDeFiAggregatorSecure");
    ...
    describe("🔒 Security Tests", function () {
        describe("Access Control", function () {
            it("Should prevent non-owner from adding protocols", async function () {
                await expect(
                    contract.connect(user1).addProtocol(
                        "malicious_protocol",
                        user1.address,
                        await mockToken.getAddress(),
                        "lending",
        ...
        describe("Input Validation", function () {
            ...
            it("Should reject excessive stake amounts", async function () {
                ...
                await mockToken.connect(user1).approve(await contract.getAddress(), excessiveAmount);
        ...
        describe("Reentrancy Protection", function () {
            it("Should prevent reentrancy attacks on stake function", async function () {
                ...
                const stakeTx = await mockToken.connect(user1).approve(await contract.getAddress(), STAKE_AMOUNT);
                await stakeTx.wait();

                // Multiple rapid calls should be blocked by rate limiting
                const promise1 = contract.connect(user1).stake(
                    await mockToken.getAddress(),
                    STAKE_AMOUNT,
                    "aave_lending"
                );
        ...
        describe("Rate Limiting", function () {
            it("Should enforce rate limiting between user actions", async function () {
                await mockToken.connect(user1).approve(await contract.getAddress(), STAKE_AMOUNT * 2n);
                
                // First stake should succeed
                await contract.connect(user1).stake(
                    await mockToken.getAddress(),
                    STAKE_AMOUNT,
                    "aave_lending"
                );
        ...
        describe("Blacklist Functionality", function () {
            it("Should prevent blacklisted addresses from staking", async function () {
                ...
                await mockToken.connect(user1).approve(await contract.getAddress(), STAKE_AMOUNT);
        ...
        describe("Circuit Breaker", function () {
            it("Should disable protocol after multiple failures", async function () {
                ...
                const failureCount = await contract.getProtocolFailureCount("aave_lending");
        ...
        describe("Emergency Withdraw", function () {
            ...
            it("Should prevent multiple emergency withdraw requests", async function () {
                ...
                await expect(
                    contract.requestEmergencyWithdraw()
    ...
    describe("🧪 Functional Tests", function () {
        describe("Normal Staking Flow", function () {
            it("Should allow normal ERC20 staking", async function () {
                ...
                .withArgs(user1.address, await mockToken.getAddress(), "aave_lending", STAKE_AMOUNT, await time.latest() + 1);
            });
        ...
        describe("Withdrawal Flow", function () {
            ...
            it("Should allow immediate withdrawal with penalty", async function () {
                ...
                await expect(
                    contract.connect(user1).withdrawImmediately(
                        await mockToken.getAddress(),
                        withdrawAmount,
                        "aave_lending"
                    )
            ...
            it("Should calculate penalties correctly", async function () {
                ...
                await contract.connect(user1).withdrawImmediately(
                    await mockToken.getAddress(),
                    withdrawAmount,
                    "aave_lending"
                );
    ...
    describe("📊 Gas Optimization Tests", function () {
        it("Should have reasonable gas costs for staking", async function () {
            ...
            const tx = await contract.connect(user1).stake(
                await mockToken.getAddress(),
                STAKE_AMOUNT,
                "aave_lending"
            );
    ...
    describe("🔍 Edge Cases", function () {
        ...
        it("Should handle precision in share calculations", async function () {
            ...
            await mockToken.connect(user1).approve(await contract.getAddress(), smallAmount);
